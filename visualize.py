...
#################################
from train import initializeTracking, getEffectiveGaussian2
import pickle
import os
#################################
...
#we can get scene_data from params by using this function !!
...
def calculate_trajectories2(scene_data,impGaussiansAll):
    in_pts = [data['means3D'][impGaussiansAll].contiguous().float().cpu().numpy() for data in scene_data]
    num_lines = len(in_pts[0])
    cols = np.repeat(colormap[np.arange(len(in_pts[0])) % len(colormap)][None], traj_length, 0).reshape(-1,3)
    out_pts = []
    for t in range(len(in_pts))[traj_length:]:
...
def getEffectiveGaussianHere():
    with open('mostImp2.pkl','rb') as f:
        mostImp = pickle.load(f)
    return mostImp
################################################
...
###########################################################
#if you want to capture screenshots in the dynamic scene
def capture_screenshots(vis,dir):
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    filename = os.path.join(dir,f"ss_{timestamp}.png")
    vis.poll_events()
    vis.update_renderer()
...
def move_camera(vis,step,zoom_factor=0,direction="left"):
    view_control = vis.get_view_control()
    if direction=="left":
        view_control.translate(step,0)
    elif direction=="right":
        view_control.translate(-step,0)
    elif direction=="up":
        view_control.translate(0,step)
    elif direction=="down":
        view_control.translate(0,-step)
    elif direction=="zoom in":
        view_control.change_field_of_view(zoom_factor)
...
def visualize(seq, exp):
    ...
    ###########################################################
    effectiveGaussians = getEffectiveGaussianHere()
    ###########################################################
    linesets = None
    lines = None
    if ADDITIONAL_LINES is not None:
        if ADDITIONAL_LINES == 'trajectories':
            ...
            linesets = calculate_trajectories2(scene_data,effectiveGaussians)
            #################################################
    ...
    while True:
        ...
        ##################################################
        #uncomment any you want to use, change steps and zoom factors as you like
        #move_camera(vis,step=1,direction="right")
        #move_camera(vis,step=1,direction="left")
        #move_camera(vis,step=1,direction="up")
        #move_camera(vis,step=1,direction="down")
        #move_camera(vis,step=1,zoom_factor=0.005,direction="zoom in")
        #move_camera(vis,step=1,zoom_factor=0.05,direction="zoom out")
        
        """
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        filename = os.path.join(screenshot_dir,f"ss_{timestamp}.png")
        vis.capture_screen_image(filename)
